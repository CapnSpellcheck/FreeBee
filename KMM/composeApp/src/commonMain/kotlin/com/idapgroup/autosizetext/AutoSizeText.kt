...
private enum class SizeDecreasingStage(val value: Float) {
   Offense(0.5f),
   Defence(1.2f),
   Diplomacy(0.95f),
   Peace(Float.NaN);
...
private fun SizeDecreasingStage?.next(didOverflowHeight: Boolean) :SizeDecreasingStage {
   return when {
...
private data class InnerMetrics(
   val fontSize: TextUnit,
   val lineHeight: TextUnit,
)

private fun coerceTextUnit(
   expected: TextUnit,
   default: TextUnit
